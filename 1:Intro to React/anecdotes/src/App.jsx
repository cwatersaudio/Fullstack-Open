...
const App = () => {
	...
	const anecdoteArray = anecdotes.map((item, index) => {
		//adds vote and id property to each anecdote
		return {
			anecdote: item,
			vote: 0,
			id: index,
	...
	const [anecdoteData, updateAnecdoteData] = useState({
		anecdoteArray,
		selected: { anecdote: "No anecdote selected yet!" },
		mostVotes: {},
	}); //initializes state as an object containing an array of anecdote objects, a 'selected' object, and a 'mostVotes' object
	...
	function voteTally(arr) {
		const maxID = arr.reduce(
			(maxIndex, elem, i, arr) =>
				elem.vote > arr[maxIndex].vote ? i : maxIndex,
			0,
		);
	...
	return (
		<div>
			...
			{anecdoteData.selected.id > -1 && (
				<p>Current vote is: {anecdoteData.selected.vote}</p>
			)}
			...
			<button
				type="button"
				onClick={() => handleVote(anecdoteData.selected.id)}
			>
				Vote
			</button>
			{anecdoteData.mostVotes.id && (
